...
# Model training
    ...
    y = df['Churn']
    X_train, X_test, y_train, y_test = train_test_split(df_x, y, test_size=0.25, random_state=42, shuffle=True)
    X_train, _, tf = preprocess(X_train, numerical, categorical)
    # Apply SMOTE to the training data
    smote = SMOTE(random_state=42, sampling_strategy=0.67, k_neighbors=6)
    ...
    # Model
    model = Sequential()
    model.add(Dense(128, input_dim=X_train_res.shape[1], activation='relu', kernel_regularizer=regularizers.l1_l2(l1=0.01, l2=0.01)))
